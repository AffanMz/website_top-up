    ...
    <main>
        <div class="flex px-10 my-8">
            <div class="w-2/3 ps-5 pe-10">
                <div class="flex">
                    <button class="px-4 py-2 font-semibold rounded-md mb-3 bg-amber-300 hover:bg-amber-500"><a href="{{ route('users.createadmin') }}">&laquo;  Kembali</a></button>
            ...
            <div class="w-1/3 px-5">
                <form method="POST" action="{{ route('users.update', $cari->id) }}">
                    @csrf
                    @method('PUT');
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="text" placeholder="Input your name" name="name" id="name" class="w-full rounded-md px-3 py-3 italic" value="{{ request()->routeis('users.edit') ? $cari->name : @old('name') }}" required/>
                    </div>
                    <div class="flex flex-col">
                        <label for="phone" class="font-semibold my-8 text-2xl text-white">Phone</label>
                        <input type="text" placeholder="Input your phone number" name="phone" id="phone" class="w-full rounded-md px-3 py-3 italic" value="{{ request()->routeis('users.edit') ? $cari->phone : @old('phone') }}" required/>
                    </div>
                    <div class="flex flex-col">
                        <label for="email" class="font-semibold my-8 text-2xl text-white">Email</label>
                        <input type="email" placeholder="Input your Email" name="email" id="email" class="w-full rounded-md px-3 py-3 italic" value="{{ request()->routeis('users.edit') ? $cari->email : @old('email') }}" required/>
                    </div>
                    <div class="flex flex-col">
                        <label for="password" class="font-semibold my-8 text-2xl text-white">Password</label>
                        <input type="password" placeholder="Input your Password" name="password" id="password" class="w-full rounded-md px-3 py-3 italic" required/>
                    </div>
    

                    <input type="hidden" name="role" value="{{ request()->is('/users.edit.{id}') ? $cari->role : $cari->role  }}">
