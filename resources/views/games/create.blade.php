        ...
        <div class="flex px-10 my-8">
            ...
            <div class="w-1/3 px-5">
                <form method="POST" action="{{ route('games.store') }}" enctype="multipart/form-data"">
                    @csrf
                    <h3 class="text-white font-semibold text-2xl mb-1">Tambah game</h3>

                    <div class="flex flex-col">
                        <label for="title" class="font-semibold my-8 text-2xl text-white">Title</label>
                        <input type="text" placeholder="Input title" name="title" id="title" class="w-full rounded-md px-3 py-3 italic" value="{{ request()->is('/users.edit.{id}') ? $item->title : @old('name') }}" required/>
                    </div>
                    <div class="flex flex-col">
                        <label for="picture" class="font-semibold my-8 text-2xl text-white">Picture</label>
                        <input type="file" placeholder="Input your picture" name="picture" id="picture" class="w-full rounded-md px-3 py-3 italic" value="{{ request()->is('/users.edit.{id}') ? $item->title : @old('name') }}" required/>
                    </div>    
