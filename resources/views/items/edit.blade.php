    ...
    <main>
        <div class="flex px-10 my-8">
            <div class="w-1/2 px-10">
                <button class="px-4 py-2 font-semibold rounded-md mb-3 bg-amber-300 hover:bg-amber-500"><a href="{{ route('items.create', $cari->id_game) }}">&laquo;  Kembali</a></button>
            ...
            <div class="w-1/2 px-10">
                <form method="POST" action="{{ route('items.update', $cari->id) }}">
                    @csrf
                    @method('PUT');
                    ...
                    <div class="w-full grid grid-cols-2 gap-3">
                        <div class="">
                            <label for="spesial_item" class="cursor-pointer">
                                <input type="radio" name="info" id="spesial_item" value="Special Item" class="peer sr-only" required {{ request()->routeis('items.edit') && $cari->info == 'Special Item' ? 'checked' : '' }} />
                        ...
                        <div class="">
                            <label for="top_up" class="cursor-pointer">
                                <input type="radio" name="info" id="top_up" value="Top Up" class="peer sr-only" {{ request()->routeis('items.edit') && $cari->info == 'Top Up' ? 'checked' : '' }}/>
                                    <div class="max-w-xl flex items-center justify-center p-5 bg-white rounded-md hover:shadow ring-2 ring-transparent peer-checked:text-amber-300 peer-checked:ring-amber-200 peer-checked:ring-offset-2">
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="text" placeholder="Diamond / Special Weekly" name="title" id="title" class="w-full rounded-md px-3 py-3 italic" value="{{ request()->routeis('items.edit') ? $cari->title : @old('title') }}" required/>
                    </div>
    
                    <div class="flex flex-col">
                        <label for="value" class="font-semibold my-8 text-2xl text-white">Jumlah Diamond / Combo</label>
                        <input type="number" placeholder="5 / 10/ 50 (Diamond)  |  2x / 3x (Combo Weekly)" name="value" id="value" value="{{ request()->routeis('items.edit') ? $cari->value : @old('value') }}" class="w-full rounded-md px-3 py-3 italic" min="0" required/>
                    </div>

                    <div class="flex flex-col">
                        <label for="value_up" class="font-semibold my-8 text-2xl text-white">Tambahan Diamond</label>
                        <input type="number" placeholder="2 / 5 (Diamond Bonus)  |  Isi '0' jika Special Item" name="value_up" id="value_up" class="w-full rounded-md px-3 py-3 italic" value="{{ request()->routeis('items.edit') ? $cari->value_up : @old('value_up') }}" min="0" required/>
                    </div>
    
                    <div class="flex flex-col">
                        <label for="price" class="font-semibold my-8 text-2xl text-white">Harga Nominal</label>
                        <input type="number" placeholder="Rp.20.000" name="price" id="price" class="w-full rounded-md px-3 py-3 italic" min="0" value="{{ request()->routeis('items.edit') ? $cari->price : @old('price') }}" required/>
                    </div>

                    <input type="hidden" name="id_game" value="{{ request()->routeis('items.edit') ? $cari->id_game : @old('id_game') }}">
